...
void ofApp::update(){
    ...
    switch (status) {
        ...
        case DISPLAY_PHOTO:{
            ...
            scenes[0]->update();
            if (((displayScene*)scenes[0])->action == displayScene::MAKE_PHOTO) {
                status = TAKE_PHOTO;
                myScene02.photoTimer = ofGetElapsedTimeMillis();
                myScene02.photoAction = scene02::STAND_BY;
                myScene02.kinect.open();
                ((displayScene*)scenes[0])->reset();
            }
            
            if (((displayScene*)scenes[0])->action == displayScene::MORE_PHOTO) {
                ((displayScene*)scenes[0])->reset();
            }
